...
const useRadialSlider = (props) => {
    const { radius = 100, sliderWidth = 18, openingRadian = Math.PI / 3, thumbRadius = 18, thumbBorderWidth = 5, min = 0, max = 200, variant, step = 1, } = props;
    ...
    const angle = (radianValue * 150.0) / Math.PI;
    ...
    const startPoint = polarToCartesian(startRadian, radius, sliderWidth, thumbRadius, thumbBorderWidth);
    const endPoint = polarToCartesian(radianValue, radius, sliderWidth, thumbRadius, thumbBorderWidth);
    const marks = useMemo(() => {
        if (isMarkerVariant) {
